		...
		<!--		кнопка "отправить на согласование"			-->
					...
					<xsl:if test="document/actions/action [.='SIGN_NO']/@enable = 'true'">
						<a style="vertical-align:12px">
							...
							<img src="/SharedResources/img/iconset/delete.png" style="border:none"/>
							<font>
			...
			<table style="width:100%; border-collapse:collapse" >
				<tr>
					<td width="73%" class="formtitle">
						<xsl:call-template name="doctitleprj"/>						
					</td>
					<td style="text-align:right; font-size:0.85em">
						<font style=""><xsl:value-of select="document/fields/docversion/@caption"/>:&#xA0;<b><xsl:value-of select="document/fields/docversion"/></b></font>
						<font style="font-style: arial; margin-left:10px;">
							<xsl:value-of select="document/fields/statuscoord/@caption"/> :
						</font>
						<font style="font-style: arial">
							<b> 
								<xsl:choose>
					...
					<table width="100%" border="0">
						<tr>
							...
							<td>
								...
								<tr>
									<td class="fc">
										<xsl:value-of select="document/fields/recipients/@caption"/>&#xA0;: 
										<xsl:if test="document/@editmode= 'edit'">
											<a href="">
										...
										<xsl:for-each select="document/fields/recipients/entry">
											<input type="hidden" id="recipient" name="recipient">
												<xsl:attribute name="value">
													<xsl:value-of select="user/@attrval"/>
												</xsl:attribute>
											</input>	
										</xsl:for-each>
									</td>
								...
								<tr>
									<td class="fc">
										<xsl:value-of select="document/fields/sender/@caption"/>&#xA0;: 
									</td>
									<td>
										<table width="500px">
											<tr>
												<td style="border:1px solid #ccc">
													<xsl:value-of select="document/fields/sender"/>&#xA0;
												</td>
											</tr>
										</table>
										<input type="hidden" id="sender" name="sender">
											<xsl:attribute name="value">
												<xsl:value-of select="document/fields/sender/@attrval"/>
											</xsl:attribute>
										</input>	
									</td>
								</tr>
						...
						<!--	 Папка		 -->						
								...
								<tr>
									...
									<td style="padding-top:5">
										<div>						
											<textarea  name="briefcontent" rows="5" style="width:85%" class="rof" onfocus="fieldOnFocus(this)" onblur="fieldOnBlur(this)" tabindex="1">
												<xsl:if test="document/@editmode= 'noaccess'">
													<xsl:attribute name="disabled">true</xsl:attribute>
												</xsl:if>	
									...
									<tr>
										<td class="fc">
											<xsl:value-of select="document/captions/more/@caption"/>&#xA0;:
										</td>							
										<td style="padding-top:5">
										...
										<tr>
											...
											<td>
												&#xA0;&#xA0;<a class="gray button form" href="javascript:delCoord()">
													<span style="vertical-align: 5px; margin-left:5px"><xsl:value-of select="document/captions/removeblock/@caption"/></span>
												</a>
											</td>
										</tr>
									</table>
						...
						<xsl:for-each select="document/fields/coordblocks/entry">
							<xsl:if test="@type='327'">
								<input type="hidden" id='signer' >
									<xsl:attribute name="value">
										<xsl:value-of select="signers/entry/user/@attrval"/>
									</xsl:attribute>
								</input>
							</xsl:if>
						</xsl:for-each>							
						<input type="hidden" name="type" value="save" />  
						<input type="hidden" name="id" value="workdocprj" />
						...
						<input type="hidden" name="coordstatus" id="coordstatus">
							<xsl:attribute name="value">
								<xsl:value-of select="document/fields/coordstatus"/>
							</xsl:attribute>
						</input>  	
