...
class addressee extends _DoScript { 
	...
	public void doProcess(_Session session, _WebFormData formData, String lang) {
		...
		if (user.hasRole(["supervisor","administrator","chancellery"])){
			def newDocAction = new _Action(getLocalizedWord("Новый адресат", lang),getLocalizedWord("Новый адресат",lang), "new_glossary")
			newDocAction.setURL("Provider?type=edit&id=addressee&key=")
