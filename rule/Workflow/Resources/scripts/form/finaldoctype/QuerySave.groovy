...
class QuerySave extends _FormQuerySave {
	...
	def validate(_WebFormData webFormData){
		...
		if (webFormData.getValueSilently("name") == ""){
			localizedMsgBox("Поле \"Тип конечного документа\" не заполнено")
			return false
		}else if (webFormData.getValueSilently('name').length() > 2046){
			localizedMsgBox('Поле \'Тип конечного документа\' содержит значение превышающее 2046 символов');
			return false;
